...
# e2e runs one relay round end to end against a headless daemon with scripted
# agents: bind -> send -> report on the MCP channel -> pull -> done (#303).
# It needs no multiplexer, so CI can run it.
# TODO(#303): the TestE2E suite behind this target still drives a private
# herdr session and must be rewritten before this comment is true.
e2e:
