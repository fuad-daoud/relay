...
// one, and the result held on screen until a key.
package pick
...
// acting on any row that is not DONE: a keystroke already in flight when the
// picker opens -- most commonly Enter -- would otherwise act on a row nobody
// had read yet. DONE rows under unbind are what gc clears anyway and run at
// once.
