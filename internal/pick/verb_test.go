...
func TestRowsForFiltersPerVerb(t *testing.T) {
	...
	rep := relay.Report{Bindings: []relay.BindingStatus{
		{Name: "active", State: string(store.StateActive), Display: "ACTIVE", BuilderStatus: stubWorking},
		{Name: "blocked", State: string(store.StateNeedsYou), Display: "NEEDS YOU", BuilderStatus: stubBlocked},
		{Name: "finished", State: string(store.StateDone), Display: "DONE", BuilderStatus: stubIdle},
	}}
