...
// statsNote builds one relay->log note entry of any kind.
...
// TestBuilderTokenFromNote covers one row per note shape relay writes: a local
...
func TestRenderStatsGolden(t *testing.T) {
	want := `relay stats: since 2026-09-16 00:00 UTC
...
func TestRenderStatsEmptySections(t *testing.T) {
	want := `relay stats: all recorded rounds
