...
// of its findings. A `relay ask --role reviewer` consult is a different thing
...
// verifyPrompt is the question relay composes for a verify consult. It names
...
// findings (#144): the LAST ```relay block, whose `verdict: accepted|rejected`
...
func parseVerdict(findings []byte) (verdict string, reasons []string) {
	...
	openIdx, closeIdx, _ := findRelayBlock(lines)
...
// still tells it not to edit; relay cannot observe writes.
//
...
// returned: a reviewer relay could not start is not a round failure. Only a
...
func startVerifyConsult(ctx context.Context, rt Runtime, tx *store.Tx, b store.Binding, round int, gateLog string) (store.Binding, error) {
	...
	// crash between the two leaves something `relay reap` can see, exactly
