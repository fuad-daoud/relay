...
// The agy variables relay reads. agy rotates ANTIGRAVITY_LS_ADDRESS and
...
// (#349) -- so a relay command running inside agy is the one moment they can be
// captured.
const (
...
// after the last capture. A conversation that has not touched relay for a week
...
func (c AgyCreds) String() string {
	return fmt.Sprintf("relay.AgyCreds{conversation_id: %q, ls_address: %q, csrf_token: %s, agentapi_exe: %q, captured_at: %s}",
...
// conversationIDRe is relay's own copy of the agy conversation id rule: a
...
// localhost or 127.0.0.1 and whose port is numeric. relay refuses anything
...
// before dispatch: an agy planner runs relay constantly (send, wait, pull,
// status), and the file is therefore fresh after every agy restart as soon as
// the planner next touches relay.
//
