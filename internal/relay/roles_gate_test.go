...
// TestRolesMissingNoteWording pins §4.2's note: a shipped path's fix is `relay
...
func TestRolesMissingNoteWording(t *testing.T) {
	...
	if !strings.Contains(shipped, "run relay agent install --kind claude") {
	...
	if !strings.Contains(mixed, "run relay agent install --kind claude") || !strings.Contains(mixed, "yourself") {
