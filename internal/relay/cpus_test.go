...
func TestTwoRoundsGetDistinctCores(t *testing.T) {
	...
	b2 := bindSecond(t, rt)
	rt.Scope = &ScopeSpec{Slice: "relay.slice", CPUWeight: 100, AllowedCPUs: "0-1"}
...
func TestExhaustedPoolRunsOnWholePool(t *testing.T) {
	...
	rt, b := seedHeadless(t, fr)
	b2 := bindSecond(t, rt)
	rt.Scope = &ScopeSpec{Slice: "relay.slice", CPUWeight: 100, AllowedCPUs: "0"}
...
func TestRelaunchKeepsItsCore(t *testing.T) {
	fr := newFakeRunner()
	rt, b := seedHeadless(t, fr)
	rt.Scope = &ScopeSpec{Slice: "relay.slice", CPUWeight: 100, AllowedCPUs: "0-3"}
	one := 1
	b.RoundCPU = &one
...
func TestNoPoolWritesNoField(t *testing.T) {
	fr := newFakeRunner()
	rt, b := seedHeadless(t, fr)
	rt.Scope = &ScopeSpec{Slice: "relay.slice", CPUWeight: 100}
