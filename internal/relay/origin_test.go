...
func TestOriginLine(t *testing.T) {
	t.Run("to builder byte-exact", func(t *testing.T) {
		...
		want := `relay: round 2 · to builder "b1" · from the planner (not the human)`
		if got != want {
	...
	t.Run("to planner report byte-exact", func(t *testing.T) {
		...
		want := `relay: round 2 · to planner · about builder "b1" (not the human)`
		if got != want {
			t.Fatalf("got %q, want %q", got, want)
		}
	})
	...
	t.Run("to planner question byte-exact", func(t *testing.T) {
		...
		want := `relay: round 3 · to planner · about builder "b1" (not the human)`
		if got != want {
			t.Fatalf("got %q, want %q", got, want)
		}
	})
	...
	t.Run("to planner findings byte-exact", func(t *testing.T) {
		...
		want := `relay: consult · to planner · about builder "b1" (not the human)`
		if got != want {
			t.Fatalf("got %q, want %q", got, want)
		}
	})
...
func TestWithOrigin(t *testing.T) {
	origin := `relay: round 1 · to planner · about builder "b1" (not the human)`
