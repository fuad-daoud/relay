...
// none.
func seedHeadless(t *testing.T, f *fakePanes, fr *fakeRunner) (Runtime, store.Binding) {
	t.Helper()
...
func TestReconcileHeadlessExitReadsTheTrailerFromTheStream(t *testing.T) {
	...
	if _, err := reconcile(t, at(rt, time.Minute), b, []stubAgent{plannerAgent()}); err != nil {
...
func TestStartRoundWithoutARunnerIsErrRunnerUnavailable(t *testing.T) {
	rt, b := seedHeadless(t, &fakePanes{}, newFakeRunner())
	rt.Runner = nil
...
func TestSendHeadlessRefusesWhileThePreviousProcessIsAlive(t *testing.T) {
	fr := newFakeRunner()
	rt, _ := seedHeadless(t, &fakePanes{}, fr)
...
func TestDrainStreamRendersNewLinesInOrderAndAdvances(t *testing.T) {
	fr := newFakeRunner()
	rt, b := sentHeadless(t, &fakePanes{}, fr) // round 1 open, cursor at 1/0
	...
	again, err := reconcile(t, rt, got, []stubAgent{plannerAgent()})
	...
	got, err = reconcile(t, rt, again, []stubAgent{plannerAgent()})
	if err != nil {
		t.Fatalf("Reconcile: %v", err)
	}
	if want := "● run_command go test ./...\n  ⎿ ok\nall done\n"; readLog(t, rt) != want {
...
func TestDrainStreamWaitsForAPartialLine(t *testing.T) {
	fr := newFakeRunner()
	rt, b := sentHeadless(t, &fakePanes{}, fr)
...
func TestDrainStreamCursorSurvivesAReload(t *testing.T) {
	...
	if _, err := reconcile(t, rt, loaded, []stubAgent{plannerAgent()}); err != nil {
		t.Fatal(err)
	}
	if readLog(t, rt) != "● run_command go test ./...\n" {
...
func TestDrainStreamIsANoopBeforeAnyRound(t *testing.T) {
	fr := newFakeRunner()
	rt, b := seedHeadless(t, &fakePanes{}, fr) // bound, never sent: StreamRound 0
...
// started on fr.
func sentHeadless(t *testing.T, f *fakePanes, fr *fakeRunner) (Runtime, store.Binding) {
	t.Helper()
	rt, _ := seedHeadless(t, f, fr)
	if _, err := Send(context.Background(), rt, "webshop", writePlan(t, "do it"), SendOptions{}); err != nil {
		t.Fatalf("Send: %v", err)
...
func TestReconcileHeadlessIdleIsNotBroken(t *testing.T) {
	...
	got, err = reconcile(t, at(rt, 5*time.Minute), got, []stubAgent{plannerAgent()})
...
func TestReconcileHeadlessAliveWaits(t *testing.T) {
	...
	got, err := reconcile(t, at(rt, 10*time.Minute), b, []stubAgent{plannerAgent()})
	if err != nil {
		t.Fatalf("Reconcile: %v", err)
	}
...
func TestReconcileHeadlessStallClearsWhenStreamMoves(t *testing.T) {
	...
	cleared, err := reconcile(t, at(rt, 11*time.Minute), got, []stubAgent{plannerAgent()})
	if err != nil {
	...
	closed, err := reconcile(t, rt2, b2, []stubAgent{plannerAgent()})
	if err != nil {
...
func TestReconcileHeadlessExitWithoutReportLogsAndSwitches(t *testing.T) {
	...
	got, err := reconcile(t, at(rt, time.Minute), b, []stubAgent{plannerAgent()})
	if err != nil {
		t.Fatalf("Reconcile: %v", err)
	}
	ex := exits(t, rt)
...
func TestReconcileHeadlessStrayProcessIsKilled(t *testing.T) {
	fr := newFakeRunner()
	rt, b := seedHeadless(t, &fakePanes{}, fr) // no round open
	b.Builder.PID = 999
...
func TestReconcilePanePathUntouchedByHeadless(t *testing.T) {
	...
	touch(t, rt.Store.DonePath("webshop", 1))
	got, err := reconcile(t, rt, b, []stubAgent{plannerWith(stubWorking, false), builderAgent(stubIdle)})
...
func TestDoneHeadlessKillFailureStillMarksDone(t *testing.T) {
	fr := newFakeRunner()
	rt, _ := sentHeadless(t, &fakePanes{}, fr)
...
func TestAnswerRefusesAHeadlessBuilderBeforeAskingHerdr(t *testing.T) {
	...
	rt2, _ := seedHeadless(t, &fakePanes{}, newFakeRunner())
...
func TestStatusHeadlessWorkingShowsPidAndLogTail(t *testing.T) {
	fr := newFakeRunner()
	rt, b := sentHeadless(t, &fakePanes{agents: []stubAgent{plannerAgent()}}, fr)
	if err := os.MkdirAll(filepath.Dir(b.Builder.LogPath), 0o755); err != nil {
		t.Fatal(err)
	}
...
func TestStatusHeadlessIdle(t *testing.T) {
	fr := newFakeRunner()
	rt, _ := seedHeadless(t, &fakePanes{agents: []stubAgent{plannerAgent()}}, fr)
	rep, err := Status(context.Background(), rt)
	if err != nil {
		t.Fatalf("Status: %v", err)
	}
...
func TestStatusHeadlessExited(t *testing.T) {
	...
	fr2 := newFakeRunner()
	rt2, b2 := sentHeadless(t, &fakePanes{agents: []stubAgent{plannerAgent()}}, fr2)
	fr2.script(b2.Builder.PID, false)
...
func TestStatusHeadlessWithoutRunnerIsUnknown(t *testing.T) {
	rt, _ := sentHeadless(t, &fakePanes{agents: []stubAgent{plannerAgent()}}, newFakeRunner())
	rt.Runner = nil
	rep, err := Status(context.Background(), rt)
	if err != nil {
		t.Fatalf("Status: %v", err)
...
// hold.
func escapeFixture(t *testing.T, f *fakePanes, fr *fakeRunner, fg *fakeGit, repo string) (Runtime, store.Binding) {
	t.Helper()
...
func sentClaudeHeadless(t *testing.T, f *fakePanes, fr *fakeRunner) (Runtime, store.Binding) {
	t.Helper()
	f.agents = []stubAgent{plannerAgent()}
	rt := newRuntime(t, f)
	rt.Runner = fr
...
func TestDrainStreamRecordsSessionIDOnce(t *testing.T) {
	fr := newFakeRunner()
	rt, b := sentClaudeHeadless(t, &fakePanes{}, fr)
