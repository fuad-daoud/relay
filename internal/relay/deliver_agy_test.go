...
const (
	agyTestOrigin  = `relay: round 1 · to planner · about builder "w" (not the human)`
...
func writeAgyMessage(t *testing.T, home, id, recipient, content string, ts time.Time, undelivered bool) {
	...
	raw, err := json.Marshal(map[string]any{
		...
		"renderDetails": map[string]string{"messageTitle": "relay"},
		"content":       content,
	})
	if err != nil {
...
func TestAgyDeliverBadConversationID(t *testing.T) {
	...
	if want := "agy planner session is not a conversation id; run relay planner init inside agy"; reason != want {
...
func TestAgyDeliverNoCredsIsUnavailable(t *testing.T) {
	...
	if want := "no agy credentials for this conversation; run any relay command inside the agy session"; reason != want {
		t.Errorf("reason = %q, want %q", reason, want)
	}
	if fake.calls != 0 {
