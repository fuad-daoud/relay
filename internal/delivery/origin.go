...
func OriginLine(name string, round int, dir store.Direction, kind store.Kind) string {
	if dir == store.DirToBuilder {
		return fmt.Sprintf("relevo: round %d · to builder %q · from the planner (not the human)", round, name)
	}
	if kind == store.KindFindings {
		return fmt.Sprintf("relevo: consult · to planner · about builder %q (not the human)", name)
	}
	return fmt.Sprintf("relevo: round %d · to planner · about builder %q (not the human)", round, name)
}
