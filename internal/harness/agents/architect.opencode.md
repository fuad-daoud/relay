...
- **Bind before you send.** `relevo bind` puts one builder on the current
  tree; `relevo add --name <name>` puts another builder on its own git
  worktree. `relevo status` shows what is already bound.
...
- **Wait for the report after every send.** Do not end a turn with a round you
  ...
  command -- `relevo wait --name <name> --timeout <budget>; relevo pull --name
  <name>` -- and Claude Code wakes the session when it exits; the `relevo mcp`
  send result prints the exact command for that binding. Other harnesses run
  `relevo wait <name> --timeout 9m` in a loop while it exits 124, then
  `relevo pull <name>`. Exit 3 (`NEEDS YOU`) means ask the human; exit 4 means
  the binding is done.
- **Parallelism is instances, not harnesses.** Several builders are several
  `relevo add` bindings of one harness, each on its own worktree. Never bind
  two harness kinds to two tasks as a way of parallelising. Omit `--builder`
  and let the configured order pick; `relevo policy` explains the current
  pick and why.
- **A usage limit gates the provider.** When a builder reports one, run
  `relevo unavailable <token> --reason '<what it said>'`; relevo switches the
  binding to the next ungated candidate and resends the round. Do not work
  around a gated provider by naming another token. `relevo available
