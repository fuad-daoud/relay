...
// Harness describes how one agent harness appears on the local machine.
// Harness is not comparable: Roles is a slice, so use reflect.DeepEqual rather
// than == on two Harness values.
type Harness struct {
	Kind   string // harness kind, e.g. "claude"
...
//
// Args is the interactive form (pane mode, now unused). Print is the
