...
// StatsReport is what `relevo stats` and `relevo stats --json` report.
...
var (
	...
	// StatsGateResults is every gate result relevo records.
...
// RenderStats is `relevo stats`' text layout. Labels are left-aligned in an
...
func RenderStats(r StatsReport) string {
	...
	if r.Since == nil {
		sb.WriteString("relevo stats: all recorded rounds\n")
	} else {
		sb.WriteString("relevo stats: since " + r.Since.UTC().Format("2006-01-02 15:04") + " UTC\n")
	}
