...
func TestLandRebasesGatesPushesLogs(t *testing.T) {
	...
	if want := (remoteBranchExistsCall{Dir: b.Worktree, Remote: "origin", Branch: "relevo/x"}); fg.remoteBranchExistsCalls[0] != want {
	...
	if want := (pushCall{Dir: b.Worktree, Remote: "origin", Branch: "relevo/x", ForceWithLease: false}); fg.pushCalls[0] != want {
	...
	if res.PRURL != "" || res.PRCommand != "gh pr create --head relevo/x --base main --fill" {
	...
	if last.Note != "landed relevo/x -> main" {
		t.Errorf("land note = %q, want %q", last.Note, "landed relevo/x -> main")
	}
	if !last.Confirmed {
...
func TestLandPRWithGh(t *testing.T) {
	...
	gh := ex.calls[1]
	if got, want := strings.Join(gh.Argv, " "), "gh pr create --head relevo/x --base main --fill"; got != want {
	...
	if got, want := entries[len(entries)-1].Note, "landed relevo/x -> main pr "+url; got != want {
...
func TestLandForceWithLeaseOnReland(t *testing.T) {
	t.Run("a rebase relands with the lease", func(t *testing.T) {
		...
		want := pushCall{Dir: b.Worktree, Remote: "origin", Branch: "relevo/x", ForceWithLease: true}
	...
	t.Run("--merge is not rewritten, so no lease", func(t *testing.T) {
		...
		want := pushCall{Dir: b.Worktree, Remote: "origin", Branch: "relevo/x", ForceWithLease: false}
		if fg.pushCalls[0] != want {
			t.Errorf("Push = %+v, want %+v", fg.pushCalls[0], want)
		}
		if res.Rebased {
...
func TestLandTextIsHonestAboutWhatItDid(t *testing.T) {
	rebased := LandResult{Branch: "relevo/x", Base: "main", Rebased: true, GateResult: "pass", Pushed: true, PRCommand: "gh pr create"}
	if got, want := LandText(rebased), "landed relevo/x -> main (rebased; gate pass; pushed)\n  open the PR: gh pr create"; got != want {
		t.Errorf("LandText = %q, want %q", got, want)
	}

	merged := LandResult{Branch: "relevo/x", Base: "main", GateResult: "skipped", Pushed: true, PRURL: "https://github.com/o/r/pull/7"}
	if got, want := LandText(merged), "landed relevo/x -> main (merged; gate skipped; pushed)\n  pr: https://github.com/o/r/pull/7"; got != want {
		t.Errorf("LandText = %q, want %q", got, want)
	}
}
