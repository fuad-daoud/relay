...
// agent-name limit and so be refused at `relevo ask` (#374 §3.3). It is pure,
...
// and so be refused at `relevo ask`. It is pure, and empty when every consult
...
// useful, so relevo warns when the name is chosen rather than refusing.
//
