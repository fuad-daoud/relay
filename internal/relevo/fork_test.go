...
// and internal/relevo/status.go and internal/ui/rail.go read -- so this
...
func TestForkSuccess(t *testing.T) {
	...
	if res.Branch != "relevo/alt" {
		t.Errorf("Branch = %q, want relevo/alt", res.Branch)
	}
	...
	if storedFork.Branch != "relevo/alt" || storedFork.Base != "commit-head-123" {
		t.Errorf("stored branch/base = (%q, %q), want (relevo/alt, commit-head-123)", storedFork.Branch, storedFork.Base)
	}
	...
	if wtCall.Dir != srcCWD || wtCall.Path != expectedWT || wtCall.Branch != "relevo/alt" || wtCall.Commit != "commit-head-123" {
...
// records the branch that checkout (src.Repo) had checked out, so `relevo
