...
// relay-era state and config roots, and the relevo-era ones they become. Every
...
// guardExempt reports whether verb runs even while relay-era state sits
...
// (#292 §4). It returns nil when there is no relay-era root, or when every one
...
func refuseUnmigrated() error {
	...
	return fmt.Errorf("relevo: relay-era state at %s is not migrated; run relevo migrate --dry-run, then relevo migrate",
