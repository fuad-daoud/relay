...
func TestRefuseUnmigrated(t *testing.T) {
	t.Run("old state root with no new one refuses with the exact text", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal("refuseUnmigrated = nil, want the refusal while relay/ has no relevo/")
		}
		want := "relevo: relay-era state at " + filepath.Join(dir, "relay") +
	...
	t.Run("both roots present is not unmigrated", func(t *testing.T) {
		dir := t.TempDir()
		for _, name := range []string{"relay", "relevo"} {
