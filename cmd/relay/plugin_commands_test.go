...
// "--help"`). Reading and regexing only -- nothing executes, so the cmd/relay
...
// allowing exactly Bash(relay:*), and exactly one line in the file starts
// `relay `, whose verb is one the CLI actually dispatches. The realistic
...
func TestPluginCommandFiles(t *testing.T) {
	...
	for _, path := range paths {
		...
		if tools, ok := frontmatterField(front, "allowed-tools:"); !ok || tools != "Bash(relay:*)" {
			t.Errorf("%s: allowed-tools must be exactly `Bash(relay:*)` (got %q, present=%v)", path, tools, ok)
		}

		// Exactly one line in the file starts `relay `: the bang-fenced
		...
		if len(preambles) != 1 {
			t.Errorf("%s: %d line(s) start with `relay `, want exactly 1: %v", path, len(preambles), preambles)
			continue
		}
