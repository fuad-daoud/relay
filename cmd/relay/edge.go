...
// cmdEdge dispatches `relay edge add|list|rm` (#37): a planner-declared
// handoff between two bindings, evaluated once at the source's round close.
func cmdEdge(args []string) error {
	const usage = `usage: relay edge add <source> --when report|diff|done|gate --then send --target <binding> --prompt <file> [--mode queue|fire] [--round N]
       relay edge list <source>
       relay edge rm <source> <id>`
...
// with no herdr binary still fails on a missing flag rather than on the
...
func cmdEdgeAdd(args []string) error {
	fs := flag.NewFlagSet("relay edge add", flag.ContinueOnError)
	...
	rest := fs.Args()
	if len(rest) != 1 {
		return fmt.Errorf("usage: relay edge add <source> --when report|diff|done|gate --then send --target <binding> --prompt <file> [--mode queue|fire] [--round N]\n" +
			"edge add needs the source binding name")
	}
	source := rest[0]
	if *when == "" {
		return fmt.Errorf("relay edge add requires --when report|diff|done|gate")
	}
	if *then == "" {
		return fmt.Errorf("relay edge add requires --then send")
	}
	if *target == "" {
		return fmt.Errorf("relay edge add requires --target <binding>")
	}
	if *prompt == "" {
		return fmt.Errorf("relay edge add requires --prompt <file>")
	}
	...
	added, err := relay.AddEdge(context.Background(), rt, source, store.Edge{
		Round:  *round,
		When:   *when,
		Then:   *then,
		Target: *target,
...
func cmdEdgeList(args []string) error {
	fs := flag.NewFlagSet("relay edge list", flag.ContinueOnError)
	if err := parseFlags(fs, args); err != nil {
		return err
	}
	rest := fs.Args()
	if len(rest) != 1 {
		return fmt.Errorf("usage: relay edge list <source>\n" +
	...
	edges, err := relay.ListEdges(rt, source)
	if err != nil {
		return err
	}
	for _, e := range edges {
...
func cmdEdgeRm(args []string) error {
	fs := flag.NewFlagSet("relay edge rm", flag.ContinueOnError)
	if err := parseFlags(fs, args); err != nil {
		return err
	}
	rest := fs.Args()
	if len(rest) != 2 {
		return fmt.Errorf("usage: relay edge rm <source> <id>\n" +
	...
	if err := relay.RemoveEdge(context.Background(), rt, source, id); err != nil {
		return err
	}
