...
// be derived: with no planner the run stops before any builder is reached, so
// the derived name is never printed.
func TestAddBranchDerivesName(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "RELAY_PLANNER") {
...
func TestBuilderWhere(t *testing.T) {
	if got := builderWhere(store.Endpoint{Mode: store.ModeRemote, Server: "zen"}); got != "zen" {
