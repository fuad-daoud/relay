...
// across bindings, archived ones included, from the round logs relay already
// writes, plus the provider blocks the last 30 days of availability history
// records (#322 stage 1). It reads the same gather `relay tab` does and
...
func cmdStats(args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintln(fs.Output(), "usage: relay stats [--since 7d] [--json]")
		fs.PrintDefaults()
	}
	...
	if len(fs.Args()) != 0 {
		return fmt.Errorf("usage: relay stats [--since 7d] [--json]")
	}
	now := time.Now().UTC()
	...
	entries, err := relay.TabEntries(rt, cut, func(msg string) {
		fmt.Fprintf(os.Stderr, "relay stats: skip %s\n", msg)
	})
	if err != nil {
		return err
	}
	...
	fmt.Print(relay.RenderStats(rep))
	return nil
}
