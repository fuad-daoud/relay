...
import (
	...
	"github.com/fuad-daoud/relay/internal/setup"
)
...
func cmdInit(args []string) error {
	fs := flag.NewFlagSet("relay init", flag.ContinueOnError)
	...
	if !res.WroteCandidates && !res.WrotePolicy {
		return errors.New("~/.config/relay/candidates.json and policy.json exist; pass --force to overwrite")
	}
	...
	fmt.Printf("next: edit the model names in %s, then run: relay doctor\n", res.CandidatesPath)
	return nil
}
