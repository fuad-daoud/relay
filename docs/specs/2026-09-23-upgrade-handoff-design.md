...
- **Roles.** `harness.InstallEnv` gains `LoadManifest() (map[string]string, error)` and `SaveManifest(map[string]string) error`. A missing manifest is an empty map. `installOne`:
  ...
  - differs and (`manifest[path] == sha(existing)` or `ShippedBefore(doc, sha(existing))`) → overwrite, `OutcomeUpdated = "updated (unchanged since relay wrote it)"`, record. Under DryRun it is `OutcomeWouldUpdate = "would update"`;
...
- **Recognising older shipped copies (R3).** An install that predates the manifest has none, so the manifest alone would read every old shipped copy as a user edit. `internal/harness/agents/shipped.sha256` is embedded, and holds the sha256 of every version of every definition relay has shipped. It is generated from git history by `scripts/agents-shipped.sh --write`, and `make check` runs `--check`, which fails when a current definition's hash is missing. `ShippedBefore(doc, sha)` treats a match as unmodified. Verified on 2026-09-23: 7 old copies on the planner's machine went from "kept (differs)" to "would update".
...
- **Doctor roles.** For every harness whose binary is on PATH, a dry-run Install with the manifest. `WouldUpdate` or `WouldWrite` → Warn `role definitions are stale; the daemon refreshes them on its next start, or run relay agent install`. `KeptDiffers` → OK with the detail `differs from every copy relay has shipped (kept as your edit)`. Keep the existing agy model check.
- **MCP.**
