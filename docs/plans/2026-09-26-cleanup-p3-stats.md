# Cleanup P3 -- finish packages: internal/stats

Round of the codebase cleanup (spec: `docs/specs/2026-09-25-codebase-cleanup-design.md`,
§4, §6 and §7 phase 3). Base: `main` at `c8d3a51d`. Packages in scope:
- `internal/stats` (794 non-test lines, 746 test lines)
...
## 1b. Hard targets (per package, against the base commit, `c8d3a51d`)
...
7. `make check` (foreground) passes -- lint, comments, file size and coverage
   guard included.
8. Copy this plan to `docs/plans/2026-09-26-cleanup-p3-stats.md`; commit once.
...
The cockpit's stats view (`internal/ui/view_stats.go`) uses this package's
`Build` and formatters heavily; their names, signatures and output are frozen,
and `go test ./internal/ui/... -count=1` must still pass. Its tests were just made
parallel (`t.Parallel()`): keep every one. Do not run the whole test suite with
`-race` on a machine with less than 8 GB free: `stats.test` is memory-hungry.
