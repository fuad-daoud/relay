# Cleanup P3 -- finish packages: internal/migrate

Round of the codebase cleanup (spec: `docs/specs/2026-09-25-codebase-cleanup-design.md`,
§4, §6 and §7 phase 3). Base: `main` at `346c743`. Packages in scope:
- `internal/migrate` (1690 non-test lines, 1316 test lines)
...
## 1b. Hard targets (per package, against the base commit, `346c743`)
...
7. `make check` (foreground) passes -- lint, comments, file size and coverage
   guard included.
8. Copy this plan to `docs/plans/2026-09-26-cleanup-p3-migrate.md`; commit once.
...
`internal/migrate` is the relay->relevo migration, kept until issue 411 removes
it. Its behaviour, exported names and the paths and file names it knows about
are frozen; clean its comments, structure and tests only.
