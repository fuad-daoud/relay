# Cleanup P3 -- finish packages: internal/remote/client
...
- `internal/remote/client` (0 non-test lines, 0 test lines)
...
7. `make check` (foreground) passes -- lint, comments, file size and coverage
   guard included.
8. Copy this plan to `docs/plans/2026-09-26-cleanup-p3-remote-client.md`; commit once.
...
Only files directly in `internal/remote/client/` are in scope, not
`internal/remote/`. The wire protocol is pinned by
`internal/remote/contract_test.go`; the client must still produce exactly the
requests those goldens describe.
