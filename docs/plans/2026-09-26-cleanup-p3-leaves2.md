# Cleanup P3 -- finish packages: internal/release, internal/upgrade, internal/chatlabel
...
- `internal/release` (803 non-test lines, 1164 test lines)
- `internal/upgrade` (208 non-test lines, 226 test lines)
- `internal/chatlabel` (318 non-test lines, 320 test lines)
...
7. `make check` (foreground) passes -- lint, comments, file size and coverage
   guard included.
8. Copy this plan to `docs/plans/2026-09-26-cleanup-p3-leaves2.md`; commit once.
...
# Cleanup P3 (round 2) -- tighten: fewer comments, no growth (`internal/release`, `internal/upgrade` and `internal/chatlabel`)

Round 2 of this binding. Round 1 (the last commit on this branch) finished
`internal/release`, `internal/upgrade` and `internal/chatlabel` to the lint rules, but missed the point of the cleanup: the owner
...
- **Run every command in the foreground and wait for it.** Never a background
  ...
  headless process, and when you end your turn the process exits and the round
  is lost. Do not end your turn until the report and the done marker exist.
- Same scope and freezes as round 1: only files in `internal/release`, `internal/upgrade` and `internal/chatlabel` (plus the plan copy);
...
Round 1 left all three packages essentially unchanged (release 805 -> 804
lines, 187 -> 186 comment lines; upgrade and chatlabel identical). The §1
targets apply to each: at most half the comment lines, no growth in lines.
