# Cleanup P3 -- finish packages: internal/serve

Round of the codebase cleanup (spec: `docs/specs/2026-09-25-codebase-cleanup-design.md`,
§4, §6 and §7 phase 3). Base: `main` at `1a513081`. Packages in scope:
- `internal/serve` (3673 non-test lines, 8014 test lines)
...
## 1b. Hard targets (per package, against the base commit, `1a513081`)
...
7. `make check` (foreground) passes -- lint, comments, file size and coverage
   guard included.
8. Copy this plan to `docs/plans/2026-09-26-cleanup-p3-serve.md`; commit once.
...
`internal/serve` is the remote-builder server. Its wire protocol, routes and
`serve status --json` document are pinned by `internal/serve/contract_test.go`,
`internal/remote/contract_test.go` and `cmd/relevo/serve_contract_test.go`; those
goldens must not change. Only files directly in `internal/serve/` are in scope.
