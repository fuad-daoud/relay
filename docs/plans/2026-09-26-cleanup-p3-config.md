# Cleanup P3 -- finish packages: internal/config
...
- `internal/config` (2074 non-test lines, 1808 test lines)
...
7. `make check` (foreground) passes -- lint, comments, file size and coverage
   guard included.
8. Copy this plan to `docs/plans/2026-09-26-cleanup-p3-config.md`; commit once.
...
`internal/config` is the database-backed config store; its document format,
`config export` output (`cmd/relevo/testdata/contract/config-export.golden`) and
revision records are contracts. Its tests were just made parallel
(`t.Parallel()`): keep every one. Moving `config` off `internal/remote/client`
is a separate phase-2 round -- do not change imports here.
