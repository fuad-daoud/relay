# Plan: strings that still describe panes (#352), and codex's "try again at <date>" reset (#311, text half)
...
- `sqlite3` turns out to matter only for pane rounds, i.e. no headless or
  served opencode round reads usage through it (`internal/usage/opencode.go`,
  `exec.go`, `source.go`). Then the doctor check in §3 should be deleted, not
  reworded, and that is a decision for the planner.
...
| `doctor.go` ~587, sqlite3 missing | `not on PATH; opencode pane rounds record usage as unknown` | `not on PATH; opencode rounds record usage as unknown` |
| `doctor.go` ~592, sqlite3 present | `on PATH; opencode pane usage readable` | `on PATH; opencode usage readable` |
...
- `doctor_test.go` ~766: assert the new Detail, which contains
  `opencode rounds record usage as unknown` and does not contain `pane`.
